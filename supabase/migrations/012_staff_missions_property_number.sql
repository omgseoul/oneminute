-- Staff-created missions, creator labels, and operator-managed property numbers.
begin;

alter table public.properties add column if not exists management_number integer;
update public.properties p set management_number=1
from public.businesses b
where p.business_id=b.id and b.code='omg' and p.code='seoul-station' and p.management_number is null;
create unique index if not exists properties_management_number_key
  on public.properties(management_number) where management_number is not null;

alter table public.missions add column if not exists creator_type text not null default 'owner'
  check (creator_type in('owner','staff'));
alter table public.missions add column if not exists creator_employee_id uuid references public.employees(id);
...
begin
  ...
  return jsonb_build_object('ok',true,'property',jsonb_build_object('property_id',v_property.id,'name',v_property.name,
    'rooms',v_property.rooms,'room_types',v_property.room_types,'management_number',v_property.management_number),
...
declare v_business_id uuid;v_property_id uuid;v_employee_id uuid;v_id uuid;v_targets jsonb;v_due timestamptz;
  v_is_owner boolean:=false;v_priority text;
begin
  ...
  if v_id is null then
    insert into public.missions(business_id,property_id,title,description,timing,priority,target_employee_ids,photo_required,estimated_minutes,due_at,creator_type,creator_employee_id)
    values(v_business_id,v_property_id,btrim(p_mission->>'title'),coalesce(p_mission->>'description',''),p_mission->>'timing',v_priority,
...
revoke all on function public.save_mission(uuid,jsonb) from public;
grant execute on function public.save_mission(uuid,jsonb) to anon,authenticated;

commit;
select '숙소번호·직원 미션·생성자 표시 준비 완료' as result;
