-- Let staff read only their own attendance while owners keep property-wide access.
begin;
...
begin
  ...
  if found then
    ...
    select s.property_id,s.employee_id,p.timezone
    into v_property_id,v_employee_id,v_timezone
  ...
  return jsonb_build_object(
    'ok',true,
    'scope',case when v_is_owner then 'property' else 'self' end,
    'timezone',v_timezone,
...
commit;
select '근무자 본인 근무현황 조회 준비 완료' as result;
